...
const hitLeft = (currentCol, character) => {
	...
	if (
		character.position.x + character.width > leftSide &&
		character.oldPosition.x <= character.position.x
	) {
		// Had to add old position so it wouldnt confuse between left or right
		character.speed.x = 0;
...
const hitBottom = (currentRow, character) => {
	...
	if (
		character.position.y < bottom &&
		character.oldPosition.y > character.position.y
	) {
		character.speed.y = 0;
...
const hitRight = (currentCol, character) => {
	...
	if (
		character.position.x < rightSide &&
		character.oldPosition.x > character.position.x
	) {
		character.speed.x = 0;
...
const hitSlope = (currentCol, currentRow, character) => {
	...
	let positionX =
		character.position.x + character.width - currentCol * updatedTileSize;
	// Using y = mx + b to calculate the height of the slope of the square
	let slopeTop =
		-0.5 * positionX + updatedTileSize + currentRow * updatedTileSize;
...
function checkPosition(character) {
	...
	currentRow = Math.floor(
		(character.position.y + character.height) / updatedTileSize
	);
	positionOnMapValue =
		collisionMap[currentRow * collisionMapCols + currentCol];
